...
final class HttpClient
{
     ...
     */
    public function request(string $url, Method $httpMethod, array $postFields, array $queryParameters, array $headers, Config $config): GenericResponse
    {
        ...
        $errorMessage = mb_strlen(trim(curl_error($ch))) === 0 ? null : curl_error($ch);
        /** @var int $statusCode */
        $statusCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);

        return ResponseFactory::createResponseFromOutput(
            $url,
            $httpMethod,
            $output,
            $statusCode,
